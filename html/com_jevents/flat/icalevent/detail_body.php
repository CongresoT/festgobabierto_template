...
if( array_key_exists('row',$this->data) ){
	...
	if (isset($row)) {
        ...
        echo("<div class='jeventpage'><h2>".$row->_title."</h2></div>");
        if($row->_dup == 7) {
            echo "Martes 7 de Noviembre, ";
        }
        elseif ($row->_dup == 8) {
            echo "Martes 8 de Noviembre, ";
        }
        echo $row->start_time." - ".$row->stop_time;
        echo "<br/>".$row->_location;
        echo "<br/>".$row->_description;
